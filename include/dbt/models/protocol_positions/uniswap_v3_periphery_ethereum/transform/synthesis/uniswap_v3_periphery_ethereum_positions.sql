...
SELECT 
    ...
    cd.tick,
    cd.fee,
    `spock-main.uniswap_v3_periphery_ethereum.uniswap_v3_periphery_ethereum_decode_position`(
        1,
        cd.token_0,
        cd.token_1,
        cd.decimals_0,
...
FROM 
    {{ ref('uniswap_v3_periphery_ethereum_liquidities') }} AS pl
INNER JOIN 
    {{ ref('uniswap_v3_periphery_ethereum_pools_info') }} AS cd
ON 
    pl.pool = cd.pool
WHERE
